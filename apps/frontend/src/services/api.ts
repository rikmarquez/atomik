...
import { 
  ...
  IdentityGoal,
  CreateIdentityGoalData,
  UpdateIdentityGoalData,
...
// Identity Goals API
export const identityGoalsApi = {
  // Get all identity goals (optionally filtered by identity area)
  getAll: async (identityAreaId?: string): Promise<IdentityGoal[]> => {
    const params = identityAreaId ? { identityAreaId } : {}
    const response = await api.get<ApiResponse<IdentityGoal[]>>('/identity-goals', { params })
    return response.data.data!
  },

  // Get specific identity goal
  getById: async (id: string): Promise<IdentityGoal> => {
    const response = await api.get<ApiResponse<IdentityGoal>>(`/identity-goals/${id}`)
    return response.data.data!
  },

  // Create new identity goal
  create: async (data: CreateIdentityGoalData): Promise<IdentityGoal> => {
    const response = await api.post<ApiResponse<IdentityGoal>>('/identity-goals', data)
    return response.data.data!
  },

  // Update identity goal
  update: async (id: string, data: UpdateIdentityGoalData): Promise<IdentityGoal> => {
    const response = await api.put<ApiResponse<IdentityGoal>>(`/identity-goals/${id}`, data)
    return response.data.data!
  },

  // Update goal progress
  updateProgress: async (id: string, data: UpdateGoalProgressData): Promise<IdentityGoal> => {
    const response = await api.patch<ApiResponse<IdentityGoal>>(`/identity-goals/${id}/progress`, data)
    return response.data.data!
  },

  // Delete identity goal
  delete: async (id: string): Promise<void> => {
    await api.delete(`/identity-goals/${id}`)
  },

  // Reorder goals within an identity area
  reorder: async (identityAreaId: string, goalIds: string[]): Promise<void> => {
    await api.post(`/identity-goals/reorder/${identityAreaId}`, { goalIds })
  },
}
